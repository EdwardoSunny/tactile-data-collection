...
Teleoperation + data-collection harness for a UFactory xArm robot, controlled by an iPhone over `teledex` (ARKit pose streamed to the workstation). Optional two-finger tactile sensing (A31301 boards over USB serial) provides (a) a gripper-safety wrapper that prevents the gripper from closing further when contact exceeds threshold, and (b) per-cell force arrows overlaid on the agent + wrist camera images. Episodes get logged to a zarr dataset shaped for behavior-cloning / diffusion-policy training (see `XArmDataset` in `dataset.py`). There is no test suite, no linter, no build step — this is a script-driven Python project. Hardware required to actually run anything: an xArm at the configured IP, Intel RealSense cameras, an iPhone running the teledex client, and (optionally) two A31301 ESP32 boards.

## Running things

`collect_with_home.py` is the single entry point:
...
# Disable just the camera overlay (still apply gripper safety, still record raw images).
python collect_with_home.py --record --no-overlay
```

Recording control during a session:
- Press the **phone button** to start/stop an episode (3-second cooldown enforced). Starting an episode first homes the robot and re-anchors the phone's AR frame to the freshly homed pose, then begins recording.
...
- `--safety-threshold FLOAT`, `--safety-metric {max_abs_z,max_norm,sum_abs_z}` — tune the gripper-safety trip point.
...
2. **`RecordingThread`** (`threads.py`) — fixed-frequency (default 10 Hz) sampler. When `recording` is enabled, it pulls a fresh `env.get_obs()` (including camera frames), optionally pulls per-board tactile state via `TactileSensors.get_latest()`, optionally draws force overlays at native resolution, resizes camera images to 224×224, concatenates `[pose(6), grasp(1)]` into a 7-dim state, and calls `recorder.append(...)`. Obs is sampled **independently** of the main control loop — pose written to disk is the pose at recording-sample time, not at action-issue time.
...
Workspace bounds are hardcoded in `XArmEnvironment.__init__`: `[[250,800],[-600,600],[30,380]]` mm. Robot IP is hardcoded in `XArmConfig` (`192.168.1.223`).

### Tactile pipeline (`environment/tactile.py`)

A single module covers:
- **`TactileConfig`** — dataclass with ports, baud, taxel count, safety_metric (`max_abs_z` / `max_norm` / `sum_abs_z`), safety_threshold, `stale_after_sec`, and optional device-side `CMD,SET,UNITS,...` / `CMD,SET,RATE_HZ,...` overrides.
- **`TactileSensor(threading.Thread)`** — one per board. Owns the `pyserial.Serial`, parses frames, handles ESP32 reboots and re-syncs on `BEGIN_STREAM`. Publishes the latest frame as a dict `{xyz: (9,3), connected: (9,), device_ts_ms, host_timestamp}` under a lock.
...
Defaults for ports, baud, lag-warning threshold, safety metric, and safety threshold live in `tactile_config.py` and are exposed as CLI flags on `collect_with_home.py`. Overlay-specific constants (cell geometry, force-axis sign, wrist anchor pixels, arrow style, camera-serial → role mapping, path to `transforms_agent.npz`) also live in `tactile_config.py`.

`environment/tactile_overlay.py` projects 9 cell positions per finger from the EE frame through the current robot pose and `trc` agent-camera extrinsics to pixels, draws inward-pointing arrows scaled by the per-cell squeeze magnitude (`sign * B[FORCE_AXIS]`), and uses fixed pixel anchors for the wrist camera. Disconnected cells render as gray dots. Overlays are drawn at native camera resolution **before** the 224×224 downsample so the arrows survive in the recorded image.
...
/data
  ...
  tactile            (N, 2, 9, 3) float32   only when use_tactile=True — per-finger (Bx,By,Bz) per taxel
