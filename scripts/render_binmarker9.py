"""Render the binmarker9 overlay variant.

binmarker9 = the NINE-ARROW counterpart of binmarker. Identical information
content (per-finger BINARY CONTACT BIT ONLY — no force direction, no force
magnitude); the only difference is presentation: 9 arrows, one per sensor
cell, instead of 1 arrow at the finger center pad.

  - nine arrows per finger, one anchored at each of the 9 FK-projected pad
    cells (points9_arrow anchors), all with FIXED length
    (binmarker_common.FIXED_ARROW_LEN) and FIXED direction (FIXED_DIRECTION,
    the sensor-frame pad normal)
  - all nine visible iff that finger's contact state is ON, per the SAME tuned
    hysteresis detector as binmarker (max-cell metric, T_ON/T_OFF + debounce)
  - the 9 pad dots are always drawn (consistent with the arrow-variant family)

Pairs with render_binmarker.py to isolate ARROW COUNT with information content
held fixed. See binmarker_common.binmarker9_feed for why each cell carries the
FULL unit vector here (points9_arrow draws per-cell arrows) whereas binmarker
feeds FIXED_DIRECTION/9 (points1_arrow draws mean*9).

Usage mirrors render_binmarker.py:
    python scripts/render_binmarker9.py SRC.zarr DST.zarr
    python scripts/render_binmarker9.py SRC.zarr --in-place
"""
import argparse
import os
import shutil
...
from binmarker_common import (FIXED_ARROW_LEN, idle_window_normalization,
                              normalize_all, binmarker9_feed, contact_metric,
                              contact_states)
...
def render(src_path, dst_path, in_place):
    ...
    for start in range(0, total_n, CHUNK):
        ...
        feeds = [(binmarker9_feed(norm[g, 0], on[g, 0]),
                  binmarker9_feed(norm[g, 1], on[g, 1])) for g in range(start, end)]
            ...
            for j in range(end - start):
                ...
                if role is None:
                    ...
                    drawn = overlay.draw(
                        ...
                        fL, fR, mode="points9_arrow", is_spatial=True,
